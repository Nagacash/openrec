...
description: "Record a window, screen, or region with OpenRec's HUD — system audio, microphone, webcam, cursor modes, countdown, and native vs. browser capture."
keywords:
  - record screen
  - window capture
...
Pick a thumbnail and hit **Share**. If no source is selected when you hit record, OpenRec opens the picker first and starts recording automatically once you choose one.
...
- **Editable overlay** (default) — OpenRec draws a stylized cursor you can theme, resize, and animate in the editor.
...
- **Language** — a locale picker (13 languages) that only affects the OpenRec UI, not your recording.

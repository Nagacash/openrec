...
The single source of truth for an OpenRec project is `AxcutDocument`, a Zod-typed
...
| `legacyEditor` | OpenRec v2 `ProjectEditorState` passthrough | Appearance/cursor settings not yet first-class in the AI-edition schema. |
...
OpenRec's pre-merge editor stored projects as `EditorProjectData` (an envelope

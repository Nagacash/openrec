...
const OUTPUT_DIR =
	...
	path.join(os.tmpdir(), `openrec-cursor-native-${Date.now()}`);
...
function spawnPowerShell(script, { onStdout, onStderr } = {}) {
	const scriptPath = path.join(
		os.tmpdir(),
		`openrec-powershell-${process.pid}-${Date.now()}-${Math.random().toString(16).slice(2)}.ps1`,
	);
...
public static class OpenRecCursorDiagnosticInterop {
...
$standardCursors = @{
    arrow = [OpenRecCursorDiagnosticInterop]::LoadCursor([IntPtr]::Zero, [IntPtr]::new(32512))
    text = [OpenRecCursorDiagnosticInterop]::LoadCursor([IntPtr]::Zero, [IntPtr]::new(32513))
    wait = [OpenRecCursorDiagnosticInterop]::LoadCursor([IntPtr]::Zero, [IntPtr]::new(32514))
    crosshair = [OpenRecCursorDiagnosticInterop]::LoadCursor([IntPtr]::Zero, [IntPtr]::new(32515))
    'up-arrow' = [OpenRecCursorDiagnosticInterop]::LoadCursor([IntPtr]::Zero, [IntPtr]::new(32516))
    'resize-nwse' = [OpenRecCursorDiagnosticInterop]::LoadCursor([IntPtr]::Zero, [IntPtr]::new(32642))
    'resize-nesw' = [OpenRecCursorDiagnosticInterop]::LoadCursor([IntPtr]::Zero, [IntPtr]::new(32643))
    'resize-ew' = [OpenRecCursorDiagnosticInterop]::LoadCursor([IntPtr]::Zero, [IntPtr]::new(32644))
    'resize-ns' = [OpenRecCursorDiagnosticInterop]::LoadCursor([IntPtr]::Zero, [IntPtr]::new(32645))
    move = [OpenRecCursorDiagnosticInterop]::LoadCursor([IntPtr]::Zero, [IntPtr]::new(32646))
    'not-allowed' = [OpenRecCursorDiagnosticInterop]::LoadCursor([IntPtr]::Zero, [IntPtr]::new(32648))
    pointer = [OpenRecCursorDiagnosticInterop]::LoadCursor([IntPtr]::Zero, [IntPtr]::new(32649))
    'app-starting' = [OpenRecCursorDiagnosticInterop]::LoadCursor([IntPtr]::Zero, [IntPtr]::new(32650))
    help = [OpenRecCursorDiagnosticInterop]::LoadCursor([IntPtr]::Zero, [IntPtr]::new(32651))
}
...
function Get-CursorAsset($cursorHandle, $cursorId) {
    $copiedHandle = [OpenRecCursorDiagnosticInterop]::CopyIcon($cursorHandle)
    if ($copiedHandle -eq [IntPtr]::Zero) {
        return $null
    }

    $iconInfo = New-Object OpenRecCursorDiagnosticInterop+ICONINFO
    $hasIconInfo = [OpenRecCursorDiagnosticInterop]::GetIconInfo($copiedHandle, [ref]$iconInfo)
    ...
    try {
        ...
        if ($hasIconInfo) {
            if ($iconInfo.hbmMask -ne [IntPtr]::Zero) {
                [OpenRecCursorDiagnosticInterop]::DeleteObject($iconInfo.hbmMask) | Out-Null
            }
            if ($iconInfo.hbmColor -ne [IntPtr]::Zero) {
                [OpenRecCursorDiagnosticInterop]::DeleteObject($iconInfo.hbmColor) | Out-Null
            }
        }
        [OpenRecCursorDiagnosticInterop]::DestroyIcon($copiedHandle) | Out-Null
    }
}

[OpenRecCursorDiagnosticInterop]::InstallMouseHook() | Out-Null
[OpenRecCursorDiagnosticInterop]::GetAsyncKeyState(0x01) | Out-Null
...
while ($true) {
    ...
    $mouseEvents = [OpenRecCursorDiagnosticInterop]::ConsumeMouseButtonEvents()
    $cursorInfo = New-Object OpenRecCursorDiagnosticInterop+CURSORINFO
    $cursorInfo.cbSize = [Runtime.InteropServices.Marshal]::SizeOf([type][OpenRecCursorDiagnosticInterop+CURSORINFO])

    if (-not [OpenRecCursorDiagnosticInterop]::GetCursorInfo([ref]$cursorInfo)) {
    ...
    $leftButtonState = [OpenRecCursorDiagnosticInterop]::GetAsyncKeyState(0x01)
...
public static class OpenRecMouseDiagnosticInterop {
    [DllImport("user32.dll")]
    [return: MarshalAs(UnmanagedType.Bool)]
...
for ($i = 0; $i -lt $points.Count; $i++) {
    $point = $points[$i]
    [OpenRecMouseDiagnosticInterop]::SetCursorPos($point.x, $point.y) | Out-Null
    if ($i -eq [int]([Math]::Floor($points.Count / 2))) {
        [OpenRecMouseDiagnosticInterop]::mouse_event(0x0002, 0, 0, 0, [UIntPtr]::Zero)
        Start-Sleep -Milliseconds 12
        [OpenRecMouseDiagnosticInterop]::mouse_event(0x0004, 0, 0, 0, [UIntPtr]::Zero)
    }
...
<title>OpenRec native cursor diagnostic</title>
<style>
...
<body>
<main>
<h1>OpenRec native cursor diagnostic</h1>
<div class="metrics">
...
<meta charset="utf-8">
<meta name="viewport" content="width=device-width, initial-scale=1">
<title>OpenRec native cursor real capture diagnostic</title>
<style>

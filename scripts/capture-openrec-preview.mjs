...
const OUTPUT_DIR =
	process.env.OPENREC_PREVIEW_OUTPUT_DIR ??
	path.join(os.tmpdir(), `openrec-real-preview-${Date.now()}`);
const FRAME_COUNT = Number(process.env.OPENREC_PREVIEW_FRAME_COUNT ?? 90);
const FPS = Number(process.env.OPENREC_PREVIEW_FPS ?? 30);
...
function findLatestCursorRecordingData() {
	...
	const candidates = fs
		...
		.filter((entry) => entry.isDirectory() && entry.name.startsWith("openrec-cursor-native-"))
...
function runNpmBuildViteIfRequested() {
	if (process.env.OPENREC_PREVIEW_SKIP_BUILD === "true") {
		ensureBuildExists();
...
const fixtureVideoPath = path.join(OUTPUT_DIR, "openrec-preview-fixture.webm");
const outputVideoPath = path.join(OUTPUT_DIR, "openrec-preview.webm");
...
try {
	...
	await hudWindow.evaluate(async () => {
		...
		throw new Error("Timed out waiting for OpenRec IPC handlers.");
	});

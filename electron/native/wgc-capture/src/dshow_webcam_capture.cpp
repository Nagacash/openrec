...
bool DirectShowWebcamCapture::initialize(
    ...
    if (!succeeded(impl_->graph->AddFilter(impl_->captureFilter.Get(), L"OpenRec Webcam Source"),
    ...
    if (!succeeded(impl_->graph->AddFilter(impl_->sampleGrabberFilter.Get(), L"OpenRec Webcam Sample Grabber"),
    ...
    if (!succeeded(impl_->graph->AddFilter(impl_->nullRenderer.Get(), L"OpenRec Webcam Null Renderer"),

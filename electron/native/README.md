...
1. `OPENREC_SCK_CAPTURE_EXE`, for local development and diagnostics.
2. `electron/native/screencapturekit/build/openrec-screencapturekit-helper`, for locally built Swift output.
3. `electron/native/bin/darwin-arm64/openrec-screencapturekit-helper` or `electron/native/bin/darwin-x64/openrec-screencapturekit-helper`, for packaged prebuilt helpers.

The macOS cursor-shape helper is resolved from `OPENREC_MAC_CURSOR_HELPER_EXE` first, then the matching `openrec-macos-cursor-helper` binary in the same local build and packaged `electron/native/bin/darwin-${arch}` directories.
...
1. `OPENREC_WGC_CAPTURE_EXE`, for local development and diagnostics.
...
```powershell
$env:OPENREC_WGC_TEST_WEBCAM_DEVICE_NAME = "NVIDIA Broadcast"
npm run test:wgc-webcam:win
Remove-Item Env:OPENREC_WGC_TEST_WEBCAM_DEVICE_NAME
```

To validate a specific native microphone manually:

```powershell
$env:OPENREC_WGC_TEST_MICROPHONE_DEVICE_NAME = "Microphone (NVIDIA Broadcast)"
npm run test:wgc-mic:win
Remove-Item Env:OPENREC_WGC_TEST_MICROPHONE_DEVICE_NAME
```

## Linux

Linux cursor recording is handled by `openrec-pipewire-helper`
...
1. `OPENREC_LINUX_CURSOR_HELPER_EXE`, for local development and diagnostics.
2. `electron/native/pipewire-capture/build/openrec-pipewire-helper`, for a locally built binary.
3. `electron/native/bin/linux-x64/openrec-pipewire-helper` (or `linux-arm64`), for packaged prebuilt helpers.
...
```bash
OPENREC_PIPEWIRE_DEBUG=1 electron/native/bin/linux-x64/openrec-pipewire-helper '{"sampleIntervalMs":100}'
```
...
`OPENREC_PIPEWIRE_DEBUG=1` additionally reports stream state transitions, the
...
```bash
electron/native/bin/linux-x64/openrec-pipewire-helper '{"probeOnly":true}'
```

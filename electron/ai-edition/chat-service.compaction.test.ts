...
// model is handed. Both seams are mocked — `invokeOpenRecAgent` for the
...
vi.mock("./deep-agent/service", () => ({
	invokeOpenRecAgent: vi.fn(),
}));

vi.mock("./deep-agent/chat-model", () => ({
	createOpenRecChatModel: vi.fn(),
...
import { createOpenRecChatModel } from "./deep-agent/chat-model";
import { invokeOpenRecAgent } from "./deep-agent/service";
import type { LlmConfigStore } from "./llm-config-store";

const invokeMock = vi.mocked(invokeOpenRecAgent);
const chatModelMock = vi.mocked(createOpenRecChatModel);
...
function stubSummarizer(reply: string) {
	...
	chatModelMock.mockImplementation(
		async () => ({ invoke }) as unknown as Awaited<ReturnType<typeof createOpenRecChatModel>>,
	);
	return invoke;
}

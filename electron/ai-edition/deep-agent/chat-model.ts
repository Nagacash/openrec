...
// ./agent-provider-capabilities.ts) is only consumed by createOpenRecChatModel,
...
// createOpenRecChatModel normalizes the provider before calling in.
...
export interface OpenRecChatModelConfig {
	provider: string;
	model: string;
	apiKey?: string;
	baseUrl?: string;
...
export const OPENAI_COMPATIBLE_NO_AUTH_API_KEY = "openrec-openai-compatible-no-auth";
...
export async function createOpenRecChatModel(
	input: OpenRecChatModelConfig,
	...
	const config: OpenRecChatModelConfig = {
		...input,

...
describe("createOpenRecChatModel — provider aliases", () => {
	it("routes the `claude` alias to the Anthropic SDK, not the OpenAI fallback", async () => {
		const model = await createOpenRecChatModel({
			provider: "claude",
...
describe("createOpenRecChatModel — Anthropic-wire output budget", () => {
	...
	it("keeps LangChain's per-model default for known Claude slugs", async () => {
		...
		const legacy = await createOpenRecChatModel({
			provider: "anthropic",
		...
		const current = await createOpenRecChatModel({
			provider: "anthropic",

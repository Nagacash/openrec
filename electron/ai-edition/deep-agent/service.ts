...
import {
	createOpenRecChatModel,
	messageContentToText,
	messageContentToThinking,
	type OpenRecChatModelConfig,
} from "./chat-model";

export interface OpenRecAgentSink {
...
const BASE_SYSTEM_PROMPT = [
	"You are an AI video editor working inside OpenRec. The user is editing a recording.",
	...
	"",
	"Cursor telemetry: while the screen was captured, OpenRec recorded where the pointer went. assets[].hasCursorTelemetry in getCurrentDocument says which assets carry it, and getCursorTrack returns the recorded track for one of them — positions over time, and the pointer shape at each moment. What it means is yours to read.",
...
export async function invokeOpenRecAgent(args: InvokeArgs): Promise<InvokeResult> {
	...
	const chatModel = await createOpenRecChatModel(model);

...
describe("ffmpegSharedBinCandidates", () => {
	...
	beforeEach(() => {
		tmpRoot = fs.mkdtempSync(path.join(os.tmpdir(), "openrec-ffmpeg-pin-test-"));
	});
...
describe("buildCandidatePaths", () => {
	...
	beforeEach(() => {
		tmpRoot = fs.mkdtempSync(path.join(os.tmpdir(), "openrec-addon-path-test-"));
	});
...
describe("CompositorViewService ffmpeg PATH prepend", () => {
	...
	beforeEach(() => {
		tmpRoot = fs.mkdtempSync(path.join(os.tmpdir(), "openrec-compositor-test-"));
...
describe("resolveSceneAssetPaths", () => {
	...
	beforeEach(() => {
		resources = fs.mkdtempSync(path.join(os.tmpdir(), "openrec-scene-assets-"));

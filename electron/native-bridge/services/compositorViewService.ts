...
export interface CompositorViewServiceOptions {
	 ...
	 * `OPENREC_COMPOSITOR_VIEW_NODE` env var and the candidate path list.
...
export class CompositorViewService {
	...
	private ensureAddon(): CompositorViewAddon | null {
		...
		const envOverride =
			this.options.envOverride ?? process.env.OPENREC_COMPOSITOR_VIEW_NODE ?? null;
	...
	async exportMulti(
		...
		const target = outPath ?? path.join(app.getPath("temp"), "openrec-native-export.mp4");
	...
	async exportGif(
		...
		const target = outPath ?? path.join(app.getPath("temp"), "openrec-native-export.gif");

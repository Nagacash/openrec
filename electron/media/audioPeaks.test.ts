...
describe("ffmpeg resolution", () => {
	...
	it("honours the env override first", () => {
		process.env.OPENREC_FFMPEG_PATH = "/custom/ffmpeg";
		try {
			expect(ffmpegCandidates(ROOT)[0]).toBe("/custom/ffmpeg");
		} finally {
			process.env.OPENREC_FFMPEG_PATH = undefined;
		}
	});
	...
	it("skips a candidate that is a directory rather than the binary", () => {
		const here = mkdtempSync(path.join(tmpdir(), "openrec-ffmpeg-"));
	 ...
	 *
	 * `OPENREC_FFMPEG_PATH` is the vehicle because `ffmpegCandidates` puts it
	...
	describe("falling through to a later candidate", () => {
		...
		beforeEach(() => {
			here = mkdtempSync(path.join(tmpdir(), "openrec-ffmpeg-"));
			staged = path.join(
				here,
				"electron",
				"native",
		...
		afterEach(() => {
			delete process.env.OPENREC_FFMPEG_PATH;
			rmSync(here, { recursive: true, force: true });
		});

		it("passes over a leading candidate that does not exist", () => {
			process.env.OPENREC_FFMPEG_PATH = path.join(here, "nowhere", "ffmpeg");
		...
		it("passes over a leading candidate that is a directory", () => {
			...
			process.env.OPENREC_FFMPEG_PATH = decoy;
		...
		it.runIf(process.platform !== "win32" && process.getuid?.() !== 0)(
			...
			() => {
				...
				process.env.OPENREC_FFMPEG_PATH = executableOnly;

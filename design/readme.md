...
A design system extracted from the **OpenRec Editor** — an AI-native screen-recording
...
- **`OpenRec Editor v4.dc.html`** — the most complete build of the editor (chat + stage + timeline).

No external codebase or Figma was provided; the `.dc.html` source IS the ground truth. Related
earlier files (`OpenRec Editor v2/v3`, `OpenRec Recording Widget`) were left out of scope
...
- A few icons in the source are hand-tuned (the OpenRec agent avatar built from `<rect>`s, the
  waveform bars, the cursor arrow). Treat those as bespoke; use Lucide for everything standard.

### Brand mark

- **`assets/logo-icon.png`** — the OpenRec app icon, transparent (24×24 in the topbar; the
  ...
  "OpenRec" in Geist 600, 15px, `-0.015em` tracking next to the icon. Do not redraw or
  recolour the mark.
...
`ui_kits/editor/` — high-fidelity recreation of the OpenRec Editor (chat + stage + timeline).
